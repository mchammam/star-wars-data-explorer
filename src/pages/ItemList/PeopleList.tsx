import { useFetchPeopleList } from '../../hooks/api/useFetchPeopleList';
...
function PeopleList() {
  ...
  const {
    ...
    totalItems,
    currentPageNumber,
  } = useFetchPeopleList({ searchQuery: debouncedSearchQuery });
  ...
  return (
    ...
    >
      <>
        {items?.map((person) => (
          <PersonListItem key={person.url} person={person} />
        ))}
