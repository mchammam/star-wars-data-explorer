...
function setDocumentColorScheme(colorScheme: ColorScheme) {
...
function ToggleColorSchemeButton() {
  ...
  useEffect(() => setDocumentColorScheme(colorScheme), []);

  function handleToggleColorSchemeClick() {
    const newColorScheme = colorScheme === 'dark' ? 'light' : 'dark';
    localStorage.setItem('colorTheme', newColorScheme);
    setColorScheme(newColorScheme);
    setDocumentColorScheme(newColorScheme);
  }

  return (
    <button
      className={styles.colorSchemeToggleBtn}
      onClick={handleToggleColorSchemeClick}
    >
      {colorScheme === 'dark' ? (
        <>
          <span className="visually-hidden">Light mode</span>
          <svg
            className={`${styles.toggleColorSchemeIcon} ${styles.lightModeIcon}`}
            viewBox="0 0 24 24"
        ...
        <>
          ...
          <svg
            className={`${styles.toggleColorSchemeIcon} ${styles.darkModeIcon}`}
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
          >
